...
def setup_google_sheets():
    ...
    try:
        ...
        if not service_account_json or not spreadsheet_id:
            logging.warning("Google Sheets Credentials oder Sheet ID fehlen")
            return None
                ...
                try:
                    ...
                    for col in pnl_columns:
                        if col in record and record[col] != '':
                            try:
                                clean_value = str(record[col]).replace('$', '').replace(',', '')
                    ...
                    for col in size_columns:
                        if col in record and record[col] != '':
                            try:
                                clean_value = str(record[col]).replace(',', '')
                    ...
                    logging.warning(f"Fehler beim Verarbeiten einer Zeile: {e}")
                    continue
